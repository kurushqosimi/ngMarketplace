-- Создание расширение citext для удобного поиска
...
-- Таблица истории цен на продукты
...
CREATE UNIQUE INDEX idx_categories_name_parent ON categories (category_name, parent_id, language) WHERE deleted_at IS NULL;
...
-- Комментарии
COMMENT
ON TABLE categories IS 'Таблица для хранения категорий товаров';
COMMENT
ON TABLE products IS 'Таблица для хранения информации о товарах маркетплейса';
COMMENT
ON TABLE product_variants IS 'Таблица для хранения вариантов товаров (например, размеры, цвета)';